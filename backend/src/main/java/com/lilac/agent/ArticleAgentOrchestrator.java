...
import com.lilac.agent.agents.*;
import com.lilac.agent.context.StreamHandlerContext;
import com.lilac.agent.parallel.ParallelImageGenerator;
import com.lilac.config.AgentConfig;
...
public class ArticleAgentOrchestrator {
    ...
    public void executePhase3_GenerateContent(ArticleState state, Consumer<String> streamHandler) {
        ...
        // 设置流式处理器到 ThreadLocal
        StreamHandlerContext.set(streamHandler);
        StreamHandlerContext.register(state.getTaskId(), streamHandler);
        ...
        try {
            ...
            StreamHandlerContext.unregister(state.getTaskId());
            StreamHandlerContext.clear();
        }
    }
