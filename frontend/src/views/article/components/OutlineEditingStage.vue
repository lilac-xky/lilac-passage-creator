            ...
            <div v-for="(section, index) in outlineSections" :key="section.section" class="outline-section"
                ...
                <div class="section-points">
                    <div v-for="(point, pointIdx) in section.points" :key="pointIdx" class="point-item">
                        ...
                        <a-button type="text" size="small" @click="deletePoint(index, pointIdx)"
        ...
        <div class="ai-chat-section">
            ...
            <div class="chat-input-wrapper">
                ...
                <a-button type="primary" :loading="aiModifying" :disabled="!modifySuggestion.trim()"
                    @click="handleAiModify" class="ai-modify-btn">
                    <template #icon>
                        <RobotOutlined />
                    </template>
                    AI 修改大纲
...
onMounted(() => {
    nextTick(() => {
        if (outlineListRef.value) {
            sortable = Sortable.create(outlineListRef.value, {
                animation: 150,
                handle: '.drag-handle',
                onEnd: (evt: SortableEvent) => {
                    const { oldIndex, newIndex } = evt
                    if (oldIndex !== undefined && newIndex !== undefined) {
                        const [item] = outlineSections.value.splice(oldIndex, 1)
                        if (!item) return
